...
class SinogramGenerator(Dataset):
    ...
    def __init__(self, binsimu, dest_path='./', length=10, image_size=(256,256), voxel_size=(2,2,2), seed=None):
        ...
        self.sinogram_simulator = SinogramSimulator(binsimu=binsimu, save_castor=False)
        self.seed = seed
...
if __name__ == '__main__':
    ...
    dataset = SinogramGenerator(binsimu=binsimu, dest_path=dest_path, length=1, seed=42)
    loader = DataLoader(dataset, batch_size=1, shuffle=False)

    for i, (noisy_1, noisy_2, clean) in enumerate(loader):
        print(f'Batch {i}:')
        print(f'  Noisy 1 shape: {noisy_1.shape}, dtype: {noisy_1.dtype}')
        print(f'  Noisy 2 shape: {noisy_2.shape}, dtype: {noisy_2.dtype}')
